...
|---|---|---|
| Starting balance 200 coins | `src/db/models/User.ts` | |
| Roll costs 200 coins | `src/config.ts` `rollCost` | |
| Selling credits 100 coins | `src/config.ts` `sellCost` | |
| Daily grants 400 coins | `src/config.ts` `daily` | |

## Commands

| v1 | v2 | Behaviour | Proved by |
|---|---|---|---|
| `!wroll` | `/waifu roll` | d100 = 1 rolls the waifu of the day | |
| | | d100 in 2..13 is a critical roll: uniform pick from the ranked set | |
| | | d100 in 14..100 is a regular roll: uniform pick from the catalog | |
...
| `!wdaily` | `/waifu daily` | d100 = 1 multiplies by 5 and shows `:sparkles: **CRITICAL ROLL!!** :sparkles:` | |
| | | d100 in 2..21 multiplies by 2 with the same banner | |
| | | Success text `You claimed :coin: N coins` | |
| | | One claim per 10:00 window in the bot timezone | |
...
| | | Picked from ranked waifus with 1..4 stars | |
...
| | | Duplicate slugs in a list count once | |
...
| Behaviour | v1 source | Proved by |
|---|---|---|
| score = ((likes+1)/(trash+1)) * (likes+trash) | `Waifu.updateScoresAndTiers` | |
| Only characters with more than 100 total votes are ranked | same | |
| Top 1% of ranked = 5 stars | `getTier` | |
| Up to 6% = 4 stars | | |
| Up to 16% = 3 stars | | |
| Up to 26% = 2 stars | | |
| Remainder = 1 star | | |
