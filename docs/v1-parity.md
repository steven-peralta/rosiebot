...
- [ ] `/admin` is hidden from non-administrators; coins set/increment/decrement and waifu add/remove as an admin, then check `/waifu coins` and `/waifu owned` for the target; `/admin banner reroll` then `/waifu banner` shows the new series; `/admin ranking status` before and during the first walk
