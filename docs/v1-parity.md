...
| Selling credits 100 coins (kept as the unranked price; ranked waifus pay more, see departures) | `src/config.ts` `sellCost` | `domain.TestConstants_MatchV1`, `domain.TestSellPriceFor_ScalesWithStars` |
...
| Behaviour | v1 source | Proved by |
|---|---|---|
| score was ((likes+1)/(trash+1)) * (likes+trash); v2 shrinks the like share toward the site-wide average with a 100-vote prior first: `p = (likes + 100·avg) / (votes + 100)`, `score = p/(1−p) × votes` (see departures) | `Waifu.updateScoresAndTiers` | `domain.TestScore_Formula`, `domain.TestLikeShare` |
...
| `/series search` (alias `/s`) shows a series card (cover, description, characters ranked first with stars, and a Browse characters button opening the v1-style pager) instead of paging straight into characters; the banner card additionally has a select menu that opens a character's card as an ephemeral reply; the `series` option on `/waifu search` and `/waifu list` still pages characters with filters | Owner liked the banner card and asked for the same on series (`discord.TestSeriesSearch_Card`, `discord.TestSeriesSearch_DirectSlugNotFoundAndErrors`, `discord.TestSeriesSearch_Autocomplete`, `discord.TestSeriesCardEmbed_CapsList`) |
| `/admin coins set/increment/decrement` and `/admin waifu add/remove`: bot-owner-only (`BOT_OWNER_IDS`; server administrators are refused and the command is disabled when no owner is configured), guild-only, ephemeral replies, balances never go below zero, grants and revokes move no coins, every action is logged with the acting admin; `/admin banner reroll` replaces this week's banner with a series other than the current and previous ones; `/admin ranking status` shows the snapshot, next refresh and live walk progress, and `/admin ranking refresh` starts a walk immediately (`app.TestRankingService_StatusTracksProgress`, `discord.TestAdmin_RankingStatus`) | New v2 feature requested by the owner (`app.TestAdminService_Coins`, `app.TestAdminService_GrantAndRevoke`, `app.TestBannerService_RerollReplacesAndExcludesCurrentAndPrevious`, `discord.TestAdmin_Coins`, `discord.TestAdmin_Waifus`, `discord.TestAdmin_BannerReroll`, `discord.TestAdmin_GuardsAndAutocomplete`, `postgres.TestRepo_EnsureAndGetPlayer`, `postgres.TestBannerStore`) |
...
| List rows (compact owned view, series and banner cards, sellall confirmation) show a five-slot ★/☆ bar so names align; card titles keep the `:star:` emoji line | Owner decision (`discord.TestStarBar`, `discord.TestOwned_CompactView`) |
| Favorites: a 🤍 Favorite button (💔 Unfavorite when the viewer already has it; the card flips after a press) on every waifu card (pagers, rolls, random, today) and series card toggles a per-guild favorite, stored with name and picture so `/favs waifus [user] [view]` and `/favs series [user]` render without API calls; the compact list is the default and `view:cards` pages through cards | Owner request; v1 had a single `favorite_waifu` field that no command used (`app.TestFavoriteService_ToggleAndList`, `memory.TestFavoriteStore`, `postgres.TestFavoriteStore`, `discord.TestFavorites_ButtonToggleAndLists`, `discord.TestFavorites_Guards`, `discord.TestFavorites_RolledAndRandomCardsCarryTheButton`) |
| Favorite alerts by DM: banner picks and rerolls (favorite waifu featured or favorite series chosen), the Waifu of the Day, and another player rolling a favorite in the same server; deduplicated per event across servers, spaced 250ms apart, skipped for users with alerts off or closed DMs (a bounced DM marks them closed until they run `/favs alerts on`), delivered in the background so commands are not delayed | Owner request (`app.TestNotifications_BannerAlertsAndDedupe`, `app.TestNotifications_WotdAndRollAlerts`, `app.TestNotifications_SettingsAndClosedDMs`, `app.TestNotifications_HooksFire`, `postgres.TestAlertStoreAndFavoriteFind`, `discord.TestFavorites_AlertsOnRoll`) |
| Ranking score uses a 100-vote prior on the like share before the ratio is taken; on the 2026-09-17 snapshot this replaced 11 of 72 five-stars, removed every five-star under 1,000 votes, left the top ten unchanged, and lifted 202 waifus in five real collections while lowering 31 | Owner decision after reviewing the flukes the raw ratio let through (`domain.TestScore_Formula`) |
| Every successful roll is recorded in a `rolls` table inside the same transaction as the debit and grant; `/waifu history [user]` shows the last ten with rating, kind and age; `/profile [user]` sums coins, collection size and sell value, rating breakdown, rarest pull, rolls, favorites and server rank; `/leaderboard [by]` ranks the server by collection value, coins, size or 4★+ count with the viewer's rank appended; series cards show "Your collection" completion and `/waifu owned series:` filters a collection to one series; `/favs series` gains `view:cards` | Owner request (`app.TestStatsService_ProfileAndLeaderboard`, `app.TestRollService_RecordsHistory`, `app.TestInventoryService_HistoryAndOwnedSet`, `postgres.TestRepo_RollsAndGuildQueries`, `discord.TestProfile_Card`, `discord.TestLeaderboard_Rankings`, `discord.TestHistory_RecentRolls`, `discord.TestOwned_SeriesFilterAndCompletion`, `discord.TestFavorites_SeriesCardsView`) |
...
- [ ] `/waifu owned` pagination: first, prev, next, last, jump modal, expired menu; `view:compact` paging; `/waifu sellall` confirm and cancel
...
- [ ] `/admin` refuses a server administrator who is not in `BOT_OWNER_IDS`; coins set/increment/decrement and waifu add/remove as an admin, then check `/waifu coins` and `/waifu owned` for the target; `/admin banner reroll` then `/waifu banner` shows the new series; `/admin ranking status` before and during the first walk
...
- [ ] `/profile`, `/profile @other`, `/leaderboard` for each metric, `/waifu history` after a few rolls, `/waifu owned series:` and the completion line on `/s search`
- [ ] With a second account favorite a waifu, roll it from the first account, and check the DM arrives; `/favs alerts off` then repeat and check it does not
- [ ] Favorite a waifu from a search page and a roll, favorite a series from `/s search`, then `/favs waifus`, `/favs waifus view:cards`, `/favs series`, and another user's list
