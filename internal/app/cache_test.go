...
func TestCachedSource_PruneAndMaintenance(t *testing.T) {
	f := newCacheFixture(t)
	f.source.EXPECT().Get(mock.Anything, "old").Return(detail("old"), nil).Once()
	if _, err := f.cached.Get(f.ctx, "old"); err != nil {
		t.Fatal(err)
	}
	f.clock.Advance(app.DefaultCacheRetain + time.Hour)
	f.source.EXPECT().Get(mock.Anything, "new").Return(detail("new"), nil).Once()
	if _, err := f.cached.Get(f.ctx, "new"); err != nil {
		t.Fatal(err)
	}
	n, err := f.cached.Prune(f.ctx)
	if err != nil || n != 1 || f.store.Len() != 1 {
		t.Errorf("prune = %d %v len=%d", n, err, f.store.Len())
	}

	ctx, cancel := context.WithCancel(f.ctx)
	cancel()
	f.cached.RunMaintenance(ctx)
}

func TestCacheConfig_Defaults(t *testing.T) {
	f := newFixture(t)
	c := app.NewCachedSource(f.source, memory.NewWaifuCache(nil), f.clock, app.CacheConfig{WaifuTTL: time.Minute}, nil)
	f.source.EXPECT().Get(mock.Anything, "rem").Return(detail("rem"), nil).Once()
	if _, err := c.Get(f.ctx, "rem"); err != nil {
		t.Fatal(err)
	}
